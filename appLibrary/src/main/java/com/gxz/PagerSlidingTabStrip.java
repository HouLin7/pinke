...
//        indicatorWrap = a.getBoolean(R.styleable.PagerSlidingTabStrip_pstsIndicatorWrap, indicatorWrap);
        ...
        for (int i = 0; i < tabCount; i++) {
            ...
            for (int j = 0; j < frameLayout.getChildCount(); j++) {
                ...
                if (v instanceof TextView) {
                    ...
                    setAlpha(tabViews.get(i).get("normal"), 1);
                    setAlpha(tabViews.get(i).get("selected"), 0);

                    //set normal  Scale
                    setPivotX(frameLayout, frameLayout.getMeasuredWidth() * 0.5f);
                    setPivotY(frameLayout, frameLayout.getMeasuredHeight() * 0.5f);
                    setScaleX(frameLayout, 1f);
                    setScaleY(frameLayout, 1f);
                    ...
                    if (i == selectedPosition) {
                        setAlpha(tabViews.get(i).get("normal"), 0);
                        setAlpha(tabViews.get(i).get("selected"), 1);

                        //set select  Scale
                        setPivotX(frameLayout, frameLayout.getMeasuredWidth() * 0.5f);
                        setPivotY(frameLayout, frameLayout.getMeasuredHeight() * 0.5f);
                        setScaleX(frameLayout, 1 + zoomMax);
                        setScaleY(frameLayout, 1 + zoomMax);
                    }
                }
            }
        }
    ...
    private class PageListener implements OnPageChangeListener {
        ...
        public void onPageSelected(int position) {
            ...
            //set old view statue
            setAlpha(tabViews.get(oldPosition).get("normal"), 1);
            setAlpha(tabViews.get(oldPosition).get("selected"), 0);
            View v_old = tabsContainer.getChildAt(oldPosition);
            setPivotX(v_old, v_old.getMeasuredWidth() * 0.5f);
            setPivotY(v_old, v_old.getMeasuredHeight() * 0.5f);
            setScaleX(v_old, 1f);
            setScaleY(v_old, 1f);

            //set new view statue
            setAlpha(tabViews.get(position).get("normal"), 0);
            setAlpha(tabViews.get(position).get("selected"), 1);
            View v_new = tabsContainer.getChildAt(position);
            setPivotX(v_new, v_new.getMeasuredWidth() * 0.5f);
            setPivotY(v_new, v_new.getMeasuredHeight() * 0.5f);
            setScaleX(v_new, 1 + zoomMax);
            setScaleY(v_new, 1 + zoomMax);
    ...
    private void setPivotX(View view, float value){
        view.setPivotX(value);
    }

    private void setPivotY(View view, float value){
        view.setPivotY(value);
    }

    private void setScaleX(View view, float value){
        view.setScaleX(value);
    }

    private void setScaleY(View view, float value){
    ...
    protected void animateFadeScale(View left, View right, float positionOffset, int position) {
        if (mState != State.IDLE) {
            if (left != null) {
                setAlpha(tabViews.get(position).get("normal"), positionOffset);
                setAlpha(tabViews.get(position).get("selected"), 1 - positionOffset);
                float mScale = 1 + zoomMax - zoomMax * positionOffset;
                setPivotX(left, left.getMeasuredWidth() * 0.5f);
                setPivotY(left, left.getMeasuredHeight() * 0.5f);
                setScaleX(left, mScale);
                setScaleY(left, mScale);
            }
            if (right != null) {
                setAlpha(tabViews.get(position + 1).get("normal"), 1 - positionOffset);
                setAlpha(tabViews.get(position + 1).get("selected"), positionOffset);
                float mScale = 1 + zoomMax * positionOffset;
                setPivotX(right, right.getMeasuredWidth() * 0.5f);
                setPivotY(right, right.getMeasuredHeight() * 0.5f);
                setScaleX(right, mScale);
                setScaleY(right, mScale);
            }
        }
    }
