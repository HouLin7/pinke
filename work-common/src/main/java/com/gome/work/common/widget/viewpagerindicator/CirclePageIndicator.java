...
package com.gome.work.common.widget.viewpagerindicator;
...
public class CirclePageIndicator extends View implements PageIndicator {
    ...
    public CirclePageIndicator(Context context, AttributeSet attrs, int defStyle) {
        ...
        // defaultPageColor));
        mPaintPageFill.setColor(Color.parseColor("#ececec"));
        ...
        // defaultFillColor));
        mPaintFill.setColor(Color.parseColor("#cdcdcd"));
        mRadius = a.getDimension(R.styleable.CirclePageIndicator_radius, defaultRadius);
        mSnap = a.getBoolean(R.styleable.CirclePageIndicator_snap, defaultSnap);

        Drawable background = a.getDrawable(R.styleable.CirclePageIndicator_indicatorBackground);
        if (background != null) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mCentered) {
            longOffset += ((longSize - longPaddingBefore - longPaddingAfter) / 2.0f)
                    - (((count - 1) * (threeRadius + longPaddingBefore)) / 2.0f);
        }
        ...
        for (int iLoop = 0; iLoop < count; iLoop++) {
            float drawLong = longOffset + (iLoop * (threeRadius + longPaddingBefore));
            if (mOrientation == HORIZONTAL) {
                dX = drawLong;
                dY = shortOffset;
            } else {
        ...
        float cx = (mSnap ? mSnapPage : mCurrentPage) * (threeRadius + longPaddingBefore);
        if (!mSnap) {
