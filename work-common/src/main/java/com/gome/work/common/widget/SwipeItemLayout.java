...
public class SwipeItemLayout extends RelativeLayout {
    ...
    private void initAttrs(Context context, AttributeSet attrs) {
        final TypedArray typedArray = context.obtainStyledAttributes(attrs,
                R.styleable.BGASwipeItemLayout);
    ...
    private boolean performAdapterViewItemLongClick() {
        ...
        if (t instanceof AdapterView) {
            ...
            try {
                Method m = AbsListView.class.getDeclaredMethod("performLongPress", View.class,
                ...
                if (view.getOnItemLongClickListener() != null) {
                    handled = view.getOnItemLongClickListener().onItemLongClick(view,
                            SwipeItemLayout.this, p, vId);
                }
                if (handled) {
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        if (mSwipeDirection == SwipeDirection.Left) {
            ...
            if (mBottomModel == BottomModel.LayDown) {
                ...
                bottomLeft = mTopLeft + mTopView.getMeasuredWidth() + mTopLp.rightMargin
                        + mBottomLp.leftMargin;

                // 底部视图的left被允许的最小值
                int minBottomLeft = r - getPaddingRight() - mBottomView.getMeasuredWidth()
                        - mBottomLp.rightMargin;
                // 获取最终的left
    ...
    /**
     * 直接关闭。如果在AbsListView中删除已经打开的item时，请用该方法关闭item，否则重用item时有问题。
     * RecyclerView中可以用该方法，也可以用closeWithAnim
     */
    public void close() {
        mPreStatus = Status.Moving;
        slideTo(0);
    ...
    public boolean isOpened() {
        return (mCurrentStatus == Status.Opened)
                || (mCurrentStatus == Status.Moving && mPreStatus == Status.Opened);
    }
    ...
    public boolean isClosed() {
        return mCurrentStatus == Status.Closed
                || (mCurrentStatus == Status.Moving && mPreStatus == Status.Closed);
    }
    ...
    private void smoothSlideTo(int isOpen) {
        if (mDragHelper.smoothSlideViewTo(mTopView, getCloseOrOpenTopViewFinalLeft(isOpen),
    ...
    private ViewDragHelper.Callback mDragHelperCallback = new ViewDragHelper.Callback() {
        ...
        public int clampViewPositionVertical(View child, int top, int dy) {
            // 这里要返回控件的getPaddingTop() +
            // mTopLp.topMargin，否则有margin和padding快速滑动松手时会上下跳动
        ...
        public void onViewPositionChanged(View changedView, int left, int top, int dx, int dy) {
            ...
            // 此时顶部视图水平方向偏移量的绝对值
            int topViewHorizontalOffset = Math.abs(mTopLeft
                    - (getPaddingLeft() + mTopLp.leftMargin));
