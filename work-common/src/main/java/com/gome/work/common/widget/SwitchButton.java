...
import com.gome.work.common.utils.ColorUtils;

/**
 * SwitchButton ch 2014-09-24
 */
...
public class SwitchButton extends CompoundButton {
    ...
    private static int[] UNCHECKED_PRESSED_STATE = new int[]{
            -android.R.attr.state_checked, android.R.attr.state_enabled,
            android.R.attr.state_pressed
    };
    ...
    private OnTouchCheck onTouchCheck;
    ...
    private void init(AttributeSet attrs) {
        ...
        mProcessAnimator = ObjectAnimator.ofFloat(this, "process", 0, 0).setDuration(
        ...
        if (ta != null) {
            thumbDrawable = ta.getDrawable(R.styleable.SwitchButton_gomeThumbDrawable);
            thumbColor = ta.getColorStateList(R.styleable.SwitchButton_gomeThumbColor);
            margin = ta.getDimension(R.styleable.SwitchButton_gomeThumbMargin, margin);
            marginLeft = ta.getDimension(R.styleable.SwitchButton_gomeThumbMarginLeft, margin);
            marginRight = ta.getDimension(R.styleable.SwitchButton_gomeThumbMarginRight, margin);
            marginTop = ta.getDimension(R.styleable.SwitchButton_gomeThumbMarginTop, margin);
            marginBottom = ta.getDimension(R.styleable.SwitchButton_gomeThumbMarginBottom, margin);
            thumbWidth = ta.getDimension(R.styleable.SwitchButton_gomeThumbWidth, thumbWidth);
            thumbHeight = ta.getDimension(R.styleable.SwitchButton_gomeThumbHeight, thumbHeight);
            thumbRadius = ta.getDimension(R.styleable.SwitchButton_gomeThumbRadius,
                    Math.min(thumbWidth, thumbHeight) / 2.f);
            backRadius = ta.getDimension(R.styleable.SwitchButton_gomeBackRadius, thumbRadius
                    + density * 2f);
            backDrawable = ta.getDrawable(R.styleable.SwitchButton_gomeBackDrawable);
            backColor = ta.getColorStateList(R.styleable.SwitchButton_gomeBackColor);
            backMeasureRatio = ta.getFloat(R.styleable.SwitchButton_gomeBackMeasureRatio,
                    backMeasureRatio);
            animationDuration = ta.getInteger(R.styleable.SwitchButton_gomeAnimationDuration,
                    animationDuration);
            fadeBack = ta.getBoolean(R.styleable.SwitchButton_gomeFadeBack, true);
            tintColor = ta.getColor(R.styleable.SwitchButton_gomeTintColor, tintColor);
            textOn = ta.getString(R.styleable.SwitchButton_gomeTextOn);
            textOff = ta.getString(R.styleable.SwitchButton_gomeTextOff);
            textMarginH = ta.getDimension(R.styleable.SwitchButton_gomeTextMarginH, textMarginH);
            ta.recycle();
        }
        ...
        mBackMeasureRatio = mThumbMargin.width() >= 0 ? Math.max(backMeasureRatio, 1)
                : backMeasureRatio;
    ...
    private Layout makeLayout(CharSequence text) {
        return new StaticLayout(text, mTextPaint, (int) Math.ceil(Layout.getDesiredWidth(text,
                mTextPaint)), Layout.Alignment.ALIGN_CENTER, 1.f, 0, false);
    }
    ...
    private int measureHeight(int heightMeasureSpec) {
        ...
        int minHeight = (int) Math.max(mThumbSizeF.y, mThumbSizeF.y + mThumbMargin.top
                + mThumbMargin.right);
    ...
    private void setup() {
        ...
        if (mOnLayout != null && mOffLayout != null) {
            if (mThumbMargin.top + mThumbMargin.bottom > 0) {
                ...
                float addition = (getMeasuredHeight() - getPaddingBottom() - getPaddingTop()
                        - mThumbSizeF.y - mThumbMargin.top - mThumbMargin.bottom) / 2;
                thumbTop += addition;
            }
        }
        ...
        float textDiffWidth = Math.min(0,
                (Math.max(mThumbSizeF.x * mBackMeasureRatio, mThumbSizeF.x + mTextWidth)
                        - mThumbRectF.width() - mTextWidth) / 2);
        float textDiffHeight = Math.min(0, (mThumbRectF.height() + mThumbMargin.top
                + mThumbMargin.bottom - mTextHeight) / 2);
        mBackRectF.set(
                backLeft + textDiffWidth,
                mThumbRectF.top - mThumbMargin.top + textDiffHeight,
                backLeft + mThumbMargin.left
                        + Math.max(mThumbSizeF.x * mBackMeasureRatio, mThumbSizeF.x + mTextWidth)
                        + mThumbMargin.right - textDiffWidth,
                mThumbRectF.bottom + mThumbMargin.bottom - textDiffHeight);

        mSafeRectF.set(mThumbRectF.left, 0,
                mBackRectF.right - mThumbMargin.right - mThumbRectF.width(), 0);

        float minBackRadius = Math.min(mBackRectF.width(), mBackRectF.height()) / 2.f;
        mBackRadius = Math.min(minBackRadius, mBackRadius);

        if (mBackDrawable != null) {
            mBackDrawable.setBounds((int) mBackRectF.left, (int) mBackRectF.top,
                    (int) mBackRectF.right, (int) mBackRectF.bottom);
        }

        if (mOnLayout != null) {
            float marginOnX = mBackRectF.left
                    + (mBackRectF.width() - mThumbRectF.width() - mOnLayout.getWidth()) / 2
                    - mThumbMargin.left + mTextMarginH * (mThumbMargin.left > 0 ? 1 : -1);
            float marginOnY = mBackRectF.top + (mBackRectF.height() - mOnLayout.getHeight()) / 2;
            mTextOnRectF.set(marginOnX, marginOnY, marginOnX + mOnLayout.getWidth(), marginOnY
                    + mOnLayout.getHeight());
        }

        if (mOffLayout != null) {
            float marginOffX = mBackRectF.right
                    - (mBackRectF.width() - mThumbRectF.width() - mOffLayout.getWidth()) / 2
                    + mThumbMargin.right - mOffLayout.getWidth() - mTextMarginH
                    * (mThumbMargin.right > 0 ? 1 : -1);
            float marginOffY = mBackRectF.top + (mBackRectF.height() - mOffLayout.getHeight()) / 2;
            mTextOffRectF.set(marginOffX, marginOffY, marginOffX + mOffLayout.getWidth(),
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mIsBackUseDrawable) {
            ...
            if (mFadeBack) {
                ...
                mPaint.setARGB(colorAlpha, Color.red(mCurrBackColor), Color.green(mCurrBackColor),
                ...
                colorAlpha = colorAlpha * alpha / 255;
                mPaint.setARGB(colorAlpha, Color.red(mNextBackColor), Color.green(mNextBackColor),
        ...
        if (switchText != null && textRectF != null) {
            int alpha = (int) (255 * (getProcess() >= 0.75 ? getProcess() * 4 - 3
            ...
            colorAlpha = colorAlpha * alpha / 255;
            switchText.getPaint().setARGB(colorAlpha, Color.red(textColor), Color.green(textColor),
        ...
        if (mIsThumbUseDrawable) {
            mThumbDrawable.setBounds((int) mPresentThumbRectF.left, (int) mPresentThumbRectF.top,
                    (int) mPresentThumbRectF.right, (int) mPresentThumbRectF.bottom);
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (deltaX < mTouchSlop && deltaY < mTouchSlop && time < mClickTimeout) {
                    ...
                    if (nextStatus != isChecked()) {
                        ...
                        if (null != onTouchCheck){
                            onTouchCheck.setOnTouchCheck();
                        }
                    } else {
    ...
    public void setChecked(final boolean checked) {
        // animate before super.setChecked() become user may call setChecked
        // again in OnCheckedChangedListener
...
//        }
    ...
    public void setOnTouchCheck(OnTouchCheck onTouchCheck) {
        this.onTouchCheck = onTouchCheck;
    }

    public interface OnTouchCheck{
        void setOnTouchCheck();
    }
}
